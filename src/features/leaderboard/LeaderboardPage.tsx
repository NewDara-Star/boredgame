      ...
      {PODIUM_ORDER.map((i, slot) => {
        ...
        return (
          <motion.div key={p.id} className="flex-1 max-w-[140px] text-center"
            ...
            <p className="text-[11px] font-bold text-soft tabular-nums">{p.answered}</p>
    ...
    <div className={`piece press flex items-center gap-3 px-3 py-2.5 ${me ? "bg-pop" : ""}`}>
      ...
      <span className="min-w-0 flex-1">
        <span className="block font-bold text-[15px] truncate">
          {p.username}{me && <span className="text-soft font-black text-[10px] uppercase tracking-widest ml-1.5">you</span>}
        </span>
        <span className="block text-[11px] font-bold text-soft tabular-nums">
          {p.answered} answered
