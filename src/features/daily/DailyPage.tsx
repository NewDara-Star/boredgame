        ...
        return (
          <div key={r.user_id} className={`piece flex items-center gap-3 px-3 py-2.5 ${me ? "bg-pop" : ""}`}>
            ...
            <span className="flex-1 font-bold text-[15px] truncate">
              {r.username}
              {me && <span className="text-soft font-black text-[10px] uppercase tracking-widest ml-1.5">you</span>}
            </span>
            <span className="text-right">
              ...
              <span className="text-[10px] font-bold text-soft tabular-nums">{secs(r.ms)}</span>
            </span>
          </div>
        );
      })}
    ...
    return (
      <motion.div variants={stagger(0.07)} initial="hidden" animate="show" className="space-y-4">
        <motion.div variants={riseIn}>
          <p className="text-[10px] font-black uppercase tracking-widest text-soft">Today's round</p>
