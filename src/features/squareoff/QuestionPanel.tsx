...
export function QuestionPanel({
  ...
  return (
    <div className="grid gap-[11px]">
      {chip && <span className="chip justify-self-start text-[12px] font-extrabold bg-sky-hi text-ink rounded-full px-[9px] py-0.5">
      ...
      <motion.div variants={stagger(0.05)} initial="hidden" animate="show" className="grid gap-2">
        {options.map((opt, i) => {
          ...
          const gone = !revealed && !!out?.has(opt);
          const dim = (revealed && !isAnswer && !isMine) || gone;
          const bg = gone ? "bg-board/55 [&>*]:opacity-55"
            : !revealed
