...
function Head({ title, to, cta = "View all" }: { title: string; to?: string; cta?: string }) {
  return (
    <div className="flex items-baseline justify-between mt-7 mb-3">
      ...
      {to && (
        <Link to={to} className="text-[13px] font-black uppercase tracking-wider text-soft
      ...
      {daily.signedIn && (
        <motion.div variants={popIn} className="mt-5">
          <Link to="/daily"
            ...
            <p className="relative text-[12px] font-black uppercase tracking-widest opacity-75">
              Daily challenge
            </p>
          ...
          {GAMES.map((g) => (
            <motion.div key={g.slug} variants={riseIn} className="snap-start shrink-0 w-[168px]">
              <Link to={g.path} className="piece press flex flex-col h-full p-4">
                ...
                <span className={`sticker inline-block self-start whitespace-nowrap text-[12px]
                  font-black uppercase tracking-wide px-2 py-0.5 ${g.chip}`}>{g.badge}</span>
