...
| Every game ends with its own result card | `src/shared/card/frame.ts` draws the frame, each game draws its final board into it: the 3×3 grid with the winning line, the seven columns with the four that won, the sixteen tiles in their winners' colours, the tubes, ten right/wrong dots with the score. Rendered all six at 1080² under the real Fredoka/Nunito and checked by eye; then driven for real — a Ball Sort race to session end and a Tic Tac Toe session to "The bot wins" — both screens show the card and the save button |
